...
def admin_get_movies():
    result = require_admin()
    if isinstance(result, tuple): return result
    movies = Movie.query.order_by(Movie.display_order, Movie.title).all()
    return jsonify([{
        'id':           m.id,
        'title':        m.title,
        'genre':        m.genre,
...
def admin_add_movie():
    ...
    if Movie.query.filter_by(title=title).first():
        return jsonify({'success': False, 'error': 'Ya existe una película con ese título'}), 400
    try:
        movie = Movie(
            title         = title,
            genre         = (data.get('genre') or '').strip(),
            duration      = int(data.get('duration') or 120),
            rating        = (data.get('rating') or 'PG-13').strip(),
            age_limit     = int(data.get('age_limit') or 13),
            age_label     = (data.get('age_label') or '+13').strip(),
            tags          = (data.get('tags') or '').strip(),
            director      = (data.get('director') or '').strip(),
            cast_list     = (data.get('cast_list') or '').strip(),
            language      = (data.get('language') or 'Subtitulada').strip(),
        ...
        db.session.flush()
        # Crear horarios aleatorios para la nueva película
        import random as _random
        for t in _random.sample(SHOWTIMES, _random.randint(2, 4)):
            db.session.add(Showtime(
                movie_id    = movie.id,
                hall        = _random.choice(HALLS),
                show_time   = t,
                format_type = _random.choice(FORMATS)
            ))
        db.session.commit()
        return jsonify({'success': True, 'id': movie.id, 'title': movie.title})
    except Exception as e:
        db.session.rollback()
...
@app.route('/api/admin/movies/<int:movie_id>/toggle', methods=['POST'])
def admin_toggle_movie(movie_id):
    ...
    db.session.commit()
    return jsonify({'success': True, 'is_active': movie.is_active, 'title': movie.title})
